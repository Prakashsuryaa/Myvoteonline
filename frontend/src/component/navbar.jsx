  ...
  return (
    <>
        ...
        <div className="flex items-center gap-x-6 ml-auto">
          ...
          <div className="relative" ref={languageRef}>
            <button
              ...
              className="text-black bg-[#FEFCFC] h-7 px-4 rounded-lg cursor-pointer"
            >
              {selectedLanguage}
            </button>
            {languageDropdown && (
              <ul className="absolute left-0 mt-1 w-24 bg-white shadow-md rounded-md">
                <li
                  onClick={() => {
                    setSelectedLanguage("ENGLISH");
                    setLanguageDropdown(false);
                  }}
                  className="px-3 py-1 hover:bg-[#165887] hover:text-white cursor-pointer text-black"
                >
                  ENGLISH
                </li>
                <li
      ...
      <nav className="text-[#165887] sticky top-0 w-full h-[50px] flex justify-between items-center border border-[#E8FAFC] bg-[#E8FAFC] z-50 px-8 md:px-12 lg:px-14">
        <button
          className="md:hidden text-[#165887]"
          onClick={() => setMenuOpen(!menuOpen)}
        >
          {menuOpen ? <FaTimes size={24} /> : <FaBars size={24} />}
        </button>
        <ul
          className={`absolute md:static top-[50px] left-0 w-full bg-[#E8FAFC] md:bg-transparent md:flex md:items-center md:space-x-10 text-lg font-semibold ${
            menuOpen ? "flex flex-col p-6" : "hidden md:flex"
          }`}
        >
          <li
            onClick={() => {
